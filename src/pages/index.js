...
import { Link, graphql } from "gatsby"; // Importación necesaria para el nuevo botón y para queries
...
const DevotionalView = ({ devocional, onWhatsAppClick }) => {
  ...
  if (devocional.fecha) {
    ...
    // Si la fecha es inválida, no mostrar nada
          ...
          {devocional.reflexion && (
            <div>
              ...
              <div className="text-gray-700 dark:text-gray-300 leading-relaxed whitespace-pre-line">
                {devocional.reflexion && typeof devocional.reflexion === 'object'
                  ? documentToReactComponents(devocional.reflexion)
                  : devocional.reflexion}
              </div>
            </div>
          )}
...
const IndexPage = ({ data }) => {
  ...
  const devocional = devotionalTextNode
    ? {
        ...
        reflexion: devotionalTextNode.reflection?.raw
          ? JSON.parse(devotionalTextNode.reflection.raw)
  ...
  return (
    <>
      ...
      {showWhatsAppBox && (
        ...
        >
          <div
            className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl shadow-xl p-5 w-[90vw] max-w-xs animate-fade-in flex flex-col items-center"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="mb-3 text-center font-semibold text-gray-800 dark:text-gray-100">
              {t("whatsapp_modal_title")}
              <br />
              <span className="text-sm text-gray-600 dark:text-gray-300">
                {t("whatsapp_modal_subtitle")}
              </span>
            </div>
